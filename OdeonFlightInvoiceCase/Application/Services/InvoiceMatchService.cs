    ...
    public async Task ProcessInvoiceAsync(string filePath, IInvoiceParser parser)
    {
        ...
        foreach (var line in parsedLines)
        {
            ...
            var invoiceNullSamePriceReservations = reservations
                                                    ...
                                                    .ToList();
            if (invoiceNullSamePriceReservations != null && invoiceNullSamePriceReservations.Count == line.PassengerCount)//linede daki sayı kadar aynı fiyatta faturasız koltuk var mı
            {
                matchedCount++;
            ...
            var samePriceReservations = reservations
                                        ...
                                        .Take(line.PassengerCount)
                                        .ToList();
            if (samePriceReservations != null && samePriceReservations.Count == line.PassengerCount)//invoice null olan yeterince yok. Toplamda aynı fiyattan var mı ? 
            {
